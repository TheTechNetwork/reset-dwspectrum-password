<#
.SYNOPSIS
  Playbook 1 - run on the WORKING DW Spectrum / Nx Witness server.
  Read-only. Extracts one account's password hash fields so they can be
  transplanted onto a locked-out server's account via Playbook 2.

.DESCRIPTION
  Before running this: on THIS (working) server, either
  ...
  Then run this script naming that account. It never writes anything -
  strictly a SELECT against the local system database.

.PARAMETER AccountName
  The local account name whose hash fields to extract. Default: admin

.PARAMETER OutputFile
  Where to write the JSON handoff file for Playbook 2. Default:
  .\hash-export.json (next to this script)

.PARAMETER Sqlite3Path
  Path to an existing sqlite3.exe. If omitted, the script looks for one
  next to itself, and if still not found, downloads the official
  single-file tool from sqlite.org (no installer, public domain).

.EXAMPLE
  .\playbook-1-extract-hash.ps1 -AccountName admin
#>

[CmdletBinding()]
param(
  [string]$AccountName = 'admin',
  [string]$OutputFile = (Join-Path $PSScriptRoot 'hash-export.json'),
  [string]$Sqlite3Path
)
...
function Ensure-Sqlite3 {
  ...
  if (-not $m.Success) {
    throw "Could not find the current sqlite-tools filename on the download page. Download it manually from https://sqlite.org/download.html, extract sqlite3.exe next to this script, and re-run."
  }
  $fileName = $m.Value
  ...
  if (-not $downloaded) {
    throw "Could not download $fileName from a recent year-folder on sqlite.org. Download it manually and extract sqlite3.exe next to this script."
  }
...
Write-Host "=== Playbook 1: extract hash for account '$AccountName' ==="
...
Write-Host 'Record this version - Playbook 2 needs a matching (or close) version on the target server.'
...
# actual password UPDATE in Playbook 2 uses the file+pipe technique, but
...
Write-Host 'Copy this file to the locked-out server and run Playbook 2 with it.'
Write-Host 'Treat it like a credential in transit - delete it from both machines once Playbook 2 succeeds.'
